...
def index22(request):
    ...
    return render(request, 'main.html', {"social":social, "clien":clien,"worke":worke,"testmon":testmon,"json_data": json_data_string, 'logstat':request.user.is_authenticated,'login':nope,'form': form, 'services': services,'register': CustomUserCreationForm()})
...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'main.html',context)
    return render(request, 'main.html', {"type":type,"item":item,"social":social, "clien":clien,"worke":worke,"testmon":testmon,"json_data": json_data_string, 'logstat':request.user.is_authenticated,'login':nope,'form': form, 'services': services,'register': CustomUserCreationForm()})
...
def member(request):
    ...
    if request.user.is_authenticated:
        ...
        chat_room = Room.objects.filter(online=user).first()
        messages = Message.objects.filter(room=chat_room).order_by('timestamp')
        context ={"social":social, "clien":clien,"worke":worke,"testmon":testmon,"json_data": json_data_string, 'messages': messages,'room': chat_room,'logstat':request.user.is_authenticated,'login':AuthenticationForm(),'form': form, 'memb': services,'register': CustomUserCreationForm()}
        if chat_room is not None:
            context['chat_id'] = chat_room.id
        else:
            context['chat_id'] = 0
